...
function channalList() {
  ...
  var p = new Promise((resolve, reject) => {
    client.request
      .invokeTemplate('channelList', {
        context: { freshChatUrl: domain_name, freshChatKey: api_key },
      })
      .then((data) => {
...
function validateAccountId() {
  ...
  var patt1 = new RegExp(/@[A-Za-z]{4,}/i)
...
var kakaoAccountIdPattern = new RegExp(/@[A-Za-z]{3,}/i)
...
fc_channel_list.addEventListener('click', (event) => {
  // var channelsUrl = `${freshchat_domain.value}/v2/channels?page=1&items_per_page=100&sort_order=asc&sort_by=name`
  ...
  var fcURL = freshchat_domain.value.replace('https://', '')
  console.log('fcURL', fcURL)
  var p = new Promise((resolve, reject) => {
    // client.request.get(channelsUrl, requestOptions)

    client.request
      .invokeTemplate('channelsUrl', {
        context: {
          freshChatUrl: fcURL,
...
fc_group_list.addEventListener('click', (event) => {
  // var groupUrl = `${freshchat_domain.value}/v2/groups?page=1&items_per_page=100&sort_order=asc&sort_by=name`
  // const requestOptions = {
  //   headers: {
  //     Authorization: `Bearer ${freshChat_key.value}`,
  //   },
  ...
  console.log('fcURL', fcURL)
  var p = new Promise((resolve, reject) => {
    // client.request.get(groupUrl, requestOptions)

    client.request
      .invokeTemplate('groupUrl', {
        context: {
          freshChatUrl: fcURL,
          freshChatKey: freshChat_key.value,
        },
...
async function verify() {
  var fcURL = freshchat_domain.value.replace('https://', '')
  console.log('fcURL', fcURL)
  console.log('freshChat_key.value', freshChat_key.value)
  try {
    var response = await client.request.invokeTemplate('verify', {
      context: {
        freshChatUrl: fcURL,
        freshChatKey: freshChat_key.value,
      },
